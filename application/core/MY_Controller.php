...
class G_Controller extends CI_Controller
{
	public function __construct()
	{
		...
		//json file save code start
		 ...
		 {	
			$url  = 'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&page=1'; // path to your JSON file
	   ...
	   //loop listing into coin elements
        ...
        foreach ($coinMarketData as $res) {
            ...
            $coinSupply[]          = $res->circulating_supply;
        ...
        /* For Global Data Top Header */
		//json file save code start
		 $filename=FCPATH . 'upload/json/global.json';
		 $create_time=date ("Y-m-d H:i:s", filemtime($filename));
		 $current_time=date ("Y-m-d H:i:s");
		 $to_time = strtotime($current_time);
		 $from_time = strtotime($create_time);
		 $duration= round(abs($to_time - $from_time) / 60,2);
		 if($duration<=30)
		 $global_results = file_get_contents($filename, true);
	     else
		 {		
	
		 $gurl  = 'https://api.coingecko.com/api/v3/global'; // path to your JSON file
	     $global_results  = $this->request($gurl);
		 $file = fopen(FCPATH.'upload/json/global.json','w');
         fwrite($file, $global_results);
         fclose($file);
		 }
        ...
        $data['globalData']         = json_decode($global_results);
        
        
         /* For Trending Coins */
		//json file save code start
		 $filename=FCPATH . 'upload/json/trending.json';
		 $create_time=date ("Y-m-d H:i:s", filemtime($filename));
		 $current_time=date ("Y-m-d H:i:s");
		 $to_time = strtotime($current_time);
		 $from_time = strtotime($create_time);
		 $duration= round(abs($to_time - $from_time) / 60,2);
		 if($duration<=360)
		 $trending_results = file_get_contents($filename, true);
	     else
		 {		
	
		 $turl  = 'https://api.coingecko.com/api/v3/search/trending'; // path to your JSON file
	     $trending_results  = $this->request($turl);
		 $file = fopen(FCPATH.'upload/json/trending.json','w');
         fwrite($file, $trending_results);
         fclose($file);
		 }
        ...
        $data['trendingData']         = json_decode($trending_results);
